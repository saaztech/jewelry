...
function adminimal_menu_link(array $variables) {
  $element = $variables['element'];
	$element['#attributes']['class'][] = 'link-' . drupal_clean_css_identifier(strtolower($element['#title']));
	//print_r($element);
  $sub_menu = '';
	
	$current = current_path();
	if ( strpos($current, $element['#href']) > -1 ) {
		$element['#attributes']['class'][] = 'active';
	}
	
	if ( $current == 'admin/structure/block' && $element['#href'] == 'admin/appearance' ) {
		$element['#attributes']['class'][] = 'active';
	}
	
	if ( $current == 'admin/structure/menu' && $element['#href'] == 'admin/appearance' ) {
		$element['#attributes']['class'][] = 'active';
	}
...
function adminimal_commerce_order_view_alter(&$build, $type) {
	//dpm($build);
  //dpm($build['#entity']);
	if ( !empty($build['#entity']->mail) ) {
		$mail = $build['#entity']->mail;
		$build['customer_email'] = array(
			'#type' => 'markup',
			'#markup' => '<b>Customer Email: </b> <a href="mailto:' . $mail . '">' . $mail . '</a>',
			'#weight' => -100,
		);
	}
}
